...
## Stokes 1st Problem
`MAE 5943`, `Spring 2023`

#### **Description:**

This notebook computes the Stokes 1ˢᵗ Problem (flow over a suddenly moved infinitely long flat plate from rest.)
...
Consider a stagnant, infinitely long plate that moved suddenly at a constant speed.
...
Please refer to the lecture notes for more information on the derivation of the ODE and numerical solution method.

#### **Solution Method:**

The equations above are solved using the Thomas algorithm. See the lecture notes for the details.

Feel free to ask questions!

*MAE 5943 - Unsteady Aerodynamics, Spring 2023* 
...
function stokes1st(ηmax=10, N=30, itermax=40, ϵProfile=1e-6, ϵBC=1e-6)
    ...
    println("-START-----------------------------------------")
	println("iter     errorProfile     errorBC f(η→∞)→0     ")
    println("-----------------------------------------------")
...
#	while ϵBC<=errorBC && iter<itermax
#	while ϵBC<=errorBC && iter<itermax
        iter += 1
		...
		errorProfile = maximum(abs.(fold-f))
		errorBC = abs(f[N+1]-f[N])
    ...
    if errorProfile<=ϵProfile 
        ...
        println("    The maximum change between consecutive profiles")
		println("    is less than the error criteria ϵProfile=$ϵProfile.")
	end

	if errorBC<=ϵBC
    	println("")
        println("Solution for the boundary condition converged!")
        println("    The difference between f(N) and f(N+1)")
		println("    is less than the error criteria ϵBC=$ϵBC.")
    end
...
# ╔═╡ c373354c-d206-4779-a191-f7459089fad0
stokes1st();

# ╔═╡ 07798e4e-08e2-11eb-3066-0120057d15b2
ηtest, ftest = stokes1st(10,30);
...
md"""
#### The effect of ηmax:
N is set to 30, and the error limit is set to 1e-6.
"""

# ╔═╡ 7bbba7b0-08e9-11eb-3d8c-75a72baa41fe
#	η, f = stokes1st(ηmax, N, itermax, ϵProfile, ϵBC);
begin
	η1, f1 = stokes1st(5,  500, 10, 1e-6, 1e-6);
	η2, f2 = stokes1st(10, 500, 10, 1e-6, 1e-6);
	η3, f3 = stokes1st(15, 500, 10, 1e-6, 1e-6);
end
	

# ╔═╡ d8a1b2d0-08e9-11eb-1c9a-91c4d297b41e
plot([f1, f2, f3], [η1, η2, η3],
        title = "Stokes 1st Problem - Effect of ηmax",
        label = ["ηmax = η1" "ηmax = η2" "ηmax = η3"],
        legend = :topleft,
        xlabel = "f",
        ylabel = "η",
        linewidth = 1,
...
begin
	ηN10, fN10 = stokes1st(10, 10);
	ηN20, fN20 = stokes1st(10, 20);
	ηN40, fN40 = stokes1st(10, 40);
	ηN80, fN80 = stokes1st(10, 80);
end

# ╔═╡ 5241b720-08ea-11eb-3fad-5f8164fe40f6
begin
	gr()
plot([fN10, fN20, fN40, fN80], [ηN10, ηN20, ηN40, ηN80],
        seriestype = :scatter,    
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "1f1ed05faf0d45241b923d0bae7605258b765d1f"
